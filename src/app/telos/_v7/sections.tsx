...
export function MissionGoals({ telos, missionId, onMission, onTrace, onOpenGoal, showIds, openFile }: MissionGoalsProps) {
  ...
  const missionShort: Record<string, string> = Object.fromEntries(
    telos.missions.map((m) => [m.id, m.title.split("—")[0].trim()]),
  );
  return (
    <section className="mission-goals" id="sec-mission">
      ...
      <div className="goals-grid" id="sec-goals">
        {telos.goals.map((g) => {
          ...
          const serves = (g as Goal & { serves?: readonly string[] }).serves ?? [];
          ...
          return (
            ...
            >
              ...
              {serves.length > 0 && (
                <div className="goal-serves">
                  {serves.map((mid) => (
                    <span
                      key={mid}
                      role="button"
                      tabIndex={0}
                      className="serves-chip"
                      title={"Serves " + mid}
                      onClick={(e) => {
                        e.stopPropagation();
                        onTrace(mid);
