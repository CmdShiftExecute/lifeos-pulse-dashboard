...
function tracesFor(telos: Telos, id: string | null): TracesResult {
  ...
  // Problem — down to affected Missions AND Goals
  const prob = telos.problems.find((x) => x.id === id);
  if (prob) {
    telos.missions
      .filter((m) => (m.addresses ?? []).includes(id))
      .forEach((m) => push(down, "Mission", m));
    if (prob.goals !== undefined) {
      prob.goals.forEach((gid) => push(down, "Goal", telos.goals.find((x) => x.id === gid)));
    } else {
      telos.goals
        .filter((x) => (x.addresses ?? []).includes(id))
        .forEach((x) => push(down, "Goal", x));
    }
    return { node: { kind: "Problem", id, title: prob.title, meta: prob.note }, up: [], down };
  }
  // Mission — down to ITS goals + ITS strategies (never all goals)
  const mis = telos.missions.find((x) => x.id === id);
  if (mis) {
    (mis.addresses ?? []).forEach((pid) =>
      push(up, "Problem", telos.problems.find((p) => p.id === pid)),
    );
    if (mis.goals !== undefined) {
      mis.goals.forEach((gid) => push(down, "Goal", telos.goals.find((x) => x.id === gid)));
    } else {
      // fixture fallback: no derived edges → show all goals as before
      telos.goals.forEach((g) => push(down, "Goal", g));
    }
    (mis.strategies ?? []).forEach((sid) =>
      push(down, "Strategy", telos.strategies.find((x) => x.id === sid)),
    );
    return { node: { kind: "Mission", id, title: mis.title, meta: mis.horizon }, up, down };
  }
  // Goal — up to parent mission(s) via serves + blocking challenges;
  // down to implementing strategies + feeding metrics
  const g = telos.goals.find((x) => x.id === id);
  if (g) {
    if (g.serves !== undefined) {
      g.serves.forEach((mid) => push(up, "Mission", telos.missions.find((m) => m.id === mid)));
    } else {
      // fixture fallback: no serves edge → show active missions as before
