...
export default function GrowthPage() {
  ...
  // Not-installed guard (2026-07-06): /api/life/growth returns {growth:null,
  // installed:false} when the optional Growth customization is absent — render
  // that state instead of crashing on missing fields.
  if ((data as any).installed === false || (data as any).growth === null) {
    return (
      <div className="p-8 max-w-5xl mx-auto">
        <div className="telos-card" style={{ cursor: "default" }}>
          <h2 className="font-medium">Growth module not installed</h2>
          <p className="text-sm muted">{(data as any).note ?? "Growth is an optional LifeOS customization (newsletter/audience analytics). Nothing is broken — this instance simply doesn't use it."}</p>
        </div>
      </div>
    );
  }
