    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-base font-medium text-muted-foreground mb-1">No optimize sessions</p>
          <p className="text-sm text-muted-foreground">Optimization runs will appear here when you use optimize mode</p>
        </div>
      </div>
    );
  }
        ...
        return (
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-base font-medium text-foreground truncate flex-1 min-w-0 uppercase tracking-wide">
                {session.taskDescription || session.sessionId?.slice(0, 8)}
              </span>
              {session.effortLevel && <EffortBadge effort={session.effortLevel} />}
              <span className="text-sm text-muted-foreground font-mono tabular-nums">{elapsed}</span>
            </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {p.stepSize != null && (
                    <div className="bg-white/[0.03] rounded-lg p-2">
                      <div className="flex items-center gap-1">
                        ...
                        <div className="text-[13px] text-muted-foreground uppercase tracking-wider">Step Size</div>
                      </div>
                      <div className="text-sm font-mono text-foreground">{Number(p.stepSize).toFixed(2)}</div>
                    </div>
                  )}
                  {p.regressionTolerance != null && (
                    <div className="bg-white/[0.03] rounded-lg p-2">
                      <div className="flex items-center gap-1">
                        <SlidersHorizontal className="w-2.5 h-2.5 text-muted-foreground" />
                        <div className="text-[13px] text-muted-foreground uppercase tracking-wider">Regression Tol.</div>
                      </div>
                      <div className="text-sm font-mono text-foreground">{Number(p.regressionTolerance).toFixed(2)}</div>
                    </div>
                  )}
                  {p.earlyStopPatience != null && (
                    <div className="bg-white/[0.03] rounded-lg p-2">
                      <div className="flex items-center gap-1">
                        <SlidersHorizontal className="w-2.5 h-2.5 text-muted-foreground" />
                        <div className="text-[13px] text-muted-foreground uppercase tracking-wider">Early Stop</div>
                      </div>
                      <div className="text-sm font-mono text-foreground">{p.earlyStopPatience}</div>
                    </div>
                  )}
                </div>
              );
            ...
            {session.criteria && session.criteria.length > 0 && (
              <div className="space-y-1">
                <div className="text-xs text-muted-foreground font-medium uppercase tracking-wide px-1">Criteria</div>
                {session.criteria.slice(0, 10).map((c) => (
                  <div key={c.id} className="flex items-center gap-2 px-1 py-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${
                      ...
                      c.status === "failed" ? "bg-danger" :
                      c.status === "in_progress" ? "bg-warn" :
                      "bg-surface-1"
                    }`} />
                    <span className={`text-sm truncate ${
                      c.type === "anti-criterion" ? "text-danger italic" : "text-muted-foreground"
                    }`}>
                      {c.description}
                    </span>
                  </div>
