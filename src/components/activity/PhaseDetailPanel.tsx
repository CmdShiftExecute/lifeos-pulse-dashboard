    ...
    <div className="border-t border-white/[0.06] bg-white/[0.015]">
      ...
      <div className="flex items-start gap-4 px-4 py-3">
        ...
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`px-2 py-0.5 rounded text-[14px] font-medium border ${EFFORT_COLORS[state.effortLevel || state.sla] ?? "bg-zinc-700 text-muted-foreground border-zinc-600/30"}`}>
        ...
        <div className="flex items-center gap-4 shrink-0 text-[15px]">
          <div className="flex items-center gap-1.5 text-muted-foreground">
            ...
            <span className="font-mono text-muted-foreground">{formatElapsed(totalElapsed)}</span>
          </div>
          <div className="h-4 w-px bg-white/[0.06]" />
          <div className="flex items-center gap-1.5">
            <span className="text-emerald-400 font-medium">{completedCount}</span>
            <span className="text-muted-foreground">/</span>
            <span className="text-foreground font-medium">{state.criteria.length}</span>
      ...
      <div className="flex items-center gap-4 px-4 py-2.5 border-t border-white/[0.04]">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="flex items-center gap-1.5">
              {state.agents.map((agent) => {
                const agentColor = AGENT_TYPE_COLORS[agent.agentType] ?? "bg-zinc-700/50 text-muted-foreground border-zinc-600/30";
                const statusDot =
                  agent.status === "active"
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="flex items-center gap-1">
              {state.capabilities.map((cap) => (
                <span key={cap} className="px-2 py-0.5 rounded bg-white/[0.04] text-muted-foreground text-[14px] border border-white/[0.04]">
                  {cap}
                </span>
              ))}
            </div>
        ...
        <div className="flex items-center gap-1.5 ml-auto shrink-0">
          ...
          <span className="text-muted-foreground ml-2 font-mono text-[14px]">{formatElapsed(totalElapsed)}</span>
        </div>
      </div>
    </div>
  );
