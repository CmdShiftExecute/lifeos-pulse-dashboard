...
const TIER_STYLE: Record<FreshnessData["tier"], { dot: string; text: string; border: string; bg: string; Icon: any; label: string }> = {
  fresh:   { dot: "bg-positive", text: "text-positive", border: "border-positive", bg: "bg-surface-1", Icon: CheckCircle2,   label: "Fresh" },
  aging:   { dot: "bg-warn",   text: "text-warn",   border: "border-warn",   bg: "bg-surface-1",   Icon: Clock,          label: "Aging" },
  stale:   { dot: "bg-danger",    text: "text-danger",    border: "border-danger",    bg: "bg-surface-1",    Icon: AlertTriangle,  label: "Stale" },
  unknown: { dot: "bg-surface-1",   text: "text-muted-foreground",   border: "border-hairline",   bg: "bg-surface-1",   Icon: HelpCircle,     label: "Unknown" },
};
    ...
    >
      ...
      >
        ...
        {!compact && freshness.label && freshness.label !== "No date info" && (
          <span className="text-[11px] text-muted-foreground">· {freshness.label}</span>
        )}
      </div>

      {hover && dated.length > 0 && (
        <div className="absolute right-0 top-full mt-2 z-50 w-72 rounded-lg border border-hairline bg-surface-1 backdrop-blur-sm shadow-xl p-3">
          ...
          <div className="flex flex-col gap-1">
            {top.map((f) => (
              <div key={f.name} className="flex items-center justify-between gap-2 text-xs">
                ...
                <span className="text-muted-foreground tabular-nums shrink-0">{f.date}</span>
              </div>
            ))}
          </div>
          {undated.length > 0 && (
            <div className="mt-2 pt-2 border-t border-hairline text-[11px] text-muted-foreground">
